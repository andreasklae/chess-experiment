...
_SYSTEM_PROMPT_EXTRA = (
    ...
    "**RE-READ WHEN MATERIAL CHANGES:** If a rook is captured and you drop from K+2R to K+R, "
    "you MUST re-read the wiki page for K+R mate (fence-and-opposition, different from herding). "
    "Read your current plan from prior moves — if it says 'herding pattern' and you no longer have "
    "two rooks, call `read_reference(skill_name=\"chess\", path=\"endgames/king-rook-mate.md\")` "
    "IMMEDIATELY and update your plan. Wiki pages stay in your context; re-reading them is FREE. "
